...
if ( !$_GET['new_reset_key'] && !$_POST['admin_submit_register'] && sizeof($stored_admin_login) == 2 && $pt_gen->valid_email($pt_conf['comms']['to_email']) == 'valid' ) {
...
if ( $_POST['admin_submit_register'] ) {
	...
	$register_result['error'][] = $pt_gen->valid_username( trim($_POST['set_username']) );
	...
	if ( $pt_gen->pass_strength($_POST['set_password'], 12, 40) != 'valid'  ) {
	$register_result['error'][] = $pt_gen->pass_strength($_POST['set_password'], 12, 40);
