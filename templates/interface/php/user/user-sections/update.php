...
<script>
		...
		var average_paid_notes = '<h5 class="align_center yellow tooltip_title">Calculating Average <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Price Paid Per Token</h5>'
			
			
			+'<p class="coin_info extra_margins" style="white-space: normal; "><span class="green">Total <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Paid For All Tokens</span> <span class="blue">&#247;</span> <span class="yellow">Total Tokens Purchased</span> <span class="blue">=</span> <span class="bitcoin">Average <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Price Paid Per Token</span></p>'
			
			+'<p class="coin_info extra_margins" style="white-space: normal; ">The RESULT of the above calculation <i>remains the same even AFTER you sell ANY amount, ONLY if you don\'t buy more between sells</i>. Everytime you buy more <i>after selling some</i>, re-calculate your Average <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Price Paid Per Token with this formula:</p>'
			
			+'<p class="coin_info extra_margins" style="white-space: normal; ">(<span class="green">Total <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Paid For All Tokens</span> <span class="blue">-</span> <span class="red">Total <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Received From All Sold Tokens</span>) <span class="blue">&#247;</span> <span class="yellow">Total Remaining Tokens Still Held</span> <span class="blue">=</span> <span class="bitcoin">Average <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> Price Paid Per Token</span></p>'
			
			+'<p class="coin_info extra_margins" style="white-space: normal; "><span class="bitcoin">PRO TIP:</span> <br /> When buying / selling, keep quick and dirty (yet clear) textual records of... <br />a) How much you bought / sold of what<br />b) What you paid / received in <?=strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair'])?> value<br />c) What / where you traded <br />d) Backup to USB Stick / NAS / DropBox / GoogleDrive / OneDrive / AmazonBucket <br />e) Now you\'re ready for tax season, to create spreadsheets from this data</p>'
	    ...
	    <div class='<?=$zebra_stripe?> long_list_taller' style='white-space: nowrap;'> 
	       ...
	       <input type='checkbox' value='<?=strtolower($asset_array_key)?>' id='<?=$field_var_watchonly?>' onchange='watch_toggle(this);' <?=( $ct['var']->rem_num_format($asset_amnt_val) > 0 && $ct['var']->rem_num_format($asset_amnt_val) <= $watch_only_flag_val ? 'checked' : '' )?> /> &nbsp;
			...
			<b class='blue'><?=$asset_array_val['name']?> (<?=strtoupper($asset_array_key)?>)</b> /  
					...
					$sel_pair = ( isset($asset_pair_id) ? $asset_pair_id : $ct['conf']['currency']['bitcoin_primary_currency_pair'] );
					}
					else {
					...
					?>
				     ...
				     echo $ct['asset']->btc_mrkt($ct['conf']['currency']['bitcoin_primary_currency_exchange']);
				     }
				     else {
					echo '1';
				     }
