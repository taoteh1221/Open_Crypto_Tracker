...
<?php
		...
		$parsed_gain_loss_total = preg_replace("/-/", "-" . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ], number_format($gain_loss_total, $thres_dec['max_dec'], '.', ',' ) );
...
<script>
		...
		echo '<span class="black private_data">Portfolio Value (' . strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair']) . '):</span> <span class="private_data">' . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] . $ct['var']->num_pretty($total_prim_currency_worth, $thres_dec['max_dec'], false, $thres_dec['min_dec']) . '</span>';
...
var fiat_val_content = '<h5 class="yellow tooltip_title">Portfolio Value In <?=strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair'])?></h5>'
		...
		echo ( $purchase_price_added == 1 && $lvrg_added == 1 && is_numeric($gain_loss_total) == true ? '<div class="portfolio_summary"><span class="black private_data">Leverage Included: </span>' . ( $total_prim_currency_worth_inc_lvrg >= 0 ? '<span class="green private_data">' : '<span class="red private_data">-' ) . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] . $parsed_total_prim_currency_worth_inc_lvrg . '</span></div>' : '' );
	 ...
	 <script>
		...
		var doc_title_stats = '<?=( $gain_loss_total >= 0 ? '+' . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] : '' )?><?=$parsed_gain_loss_total?> (<?=( $gain_loss_total >= 0 ? '+' : '-' )?><?=number_format($percent_difference_total, $thres_dec['max_dec'], '.', ',')?>%)';
