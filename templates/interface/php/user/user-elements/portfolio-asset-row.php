...
<td class='data border_lb' align='right' style='position: relative; white-space: nowrap;'>
 ...
 <script>
        ...
        +'<p class="coin_info" style="white-space: normal; "><span class="bitcoin">Stock market data is provided with <a href="https://www.alphavantage.co/" target="_blank">Alpha Vantage\'s API</a>.</span></p>';
			...
			<?php
        		...
        		$mcap_prim_currency_symb = $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ];
        		$mcap_prim_currency_ticker = strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair']);
        		}
 ...
 <script>
        ...
        +'<p class="coin_info" style="white-space: normal; "><span class="bitcoin">Miscellaneous <?=strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair'])?> value can be included in you portfolio stats, by entering it under the "MISCASSETS" asset on the "Update" page.</span></p>'
        
        +'<p class="coin_info" style="white-space: normal; "><span class="bitcoin">This can be useful for including <?=strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair'])?> Checking / Savings accounts at banks, stable coin holdings, etc.</span></p>'
        
        +'<p class="coin_info" style="white-space: normal; "><span class="bitcoin">Additionally, you can see it\'s potential market value in another asset by changing the "Market" value on the "Portfolio" page to an asset other than <?=strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair'])?>.</span></p>';
        ...
        +'<p class="coin_info" style="white-space: normal; "><span class="bitcoin"><?=strtoupper($ct['conf']['gen']['bitcoin_primary_currency_pair'])?> value of NFTS can be included in you portfolio stats, by entering it under the "ALTNFTS" asset on the "Update" page.</span></p>'
...
<?php
  ...
  echo "<span class='white'>" . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] . "</span>" . "<span class='app_sort_filter'>" . $asset_prim_currency_val . "</span>";
  ...
  if ( $ct['sel_opt']['show_secondary_trade_val'] != null && $sel_pair != $ct['sel_opt']['show_secondary_trade_val'] && strtolower($asset_symb) != $ct['sel_opt']['show_secondary_trade_val'] ) {
		...
		$secondary_trade_val_result = $ct['var']->num_to_str($btc_trade_eqiv_raw);
          $thres_dec = $ct['gen']->thres_dec($secondary_trade_val_result, 'u', 'crypto'); // Units mode
     		...
     		// Fiat-eqiv
            	if ( array_key_exists($ct['sel_opt']['show_secondary_trade_val'], $ct['conf']['currency']['bitcoin_currency_markets']) ) {
...
<span class='white'><?=$ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ]?></span><span class='app_sort_filter'><?php 
...
<?php
	...
	if ( strtolower($asset_symb) == 'btc' ) {
     $thres_dec = $ct['gen']->thres_dec($asset_amnt, 'u', 'crypto'); // Units mode
     	...
     	// Fiat-eqiv
          if ( array_key_exists(strtolower($asset_symb), $ct['conf']['currency']['bitcoin_currency_markets']) ) {
...
echo '<span class="private_data ' . ( $purchase_price >= $min_fiat_val_test && $lvrg_level >= 2 && $sel_mrgntyp == 'short' ? 'short">★ ' : 'blue">' ) . '<span class="blue">' . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] . '</span><span class="app_sort_filter blue">' . $ct['var']->num_pretty($asset_prim_currency_worth_raw, $thres_dec['max_dec'], false, $thres_dec['min_dec']) . '</span></span>';
  ...
  $parsed_gain_loss = preg_replace("/-/", "-" . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ], number_format($gain_loss, $thres_dec['max_dec'], '.', ',' ) );
  

  $thres_dec = $ct['gen']->thres_dec($inc_lvrg_gain_loss, 'u', 'fiat'); // Units mode
  $parsed_inc_lvrg_gain_loss = preg_replace("/-/", "-" . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ], number_format($inc_lvrg_gain_loss, $thres_dec['max_dec'], '.', ',' ) );
  
  
  $thres_dec = $ct['gen']->thres_dec($only_lvrg_gain_loss, 'u', 'fiat'); // Units mode
  $parsed_only_lvrg_gain_loss = preg_replace("/-/", "-" . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ], number_format($only_lvrg_gain_loss, $thres_dec['max_dec'], '.', ',' ) );
  ...
  $pretty_lvrg_gain_loss_percent = number_format( $inc_lvrg_gain_loss_percent, $thres_dec['max_dec'], '.', ',' );
  		...
  		$gain_loss_prim_currency = ( $gain_loss >= 0 ? '+' . $ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ] : '' );
	 ...
	 <script>
			...
			+'<p class="coin_info"><span class="bitcoin">Deposit (1x):</span> <span class="<?=$gain_loss_span_color?>"><?=$gain_loss_prim_currency?><?=$parsed_gain_loss?></span> (<?=$ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ]?><?=$pretty_asset_prim_currency_worth_raw?>)</p>'
			...
			+'<p class="coin_info"><span class="bitcoin">Total (<?=($lvrg_level)?>x):</span> <span class="<?=$gain_loss_span_color?>"><?=$gain_loss_prim_currency?><?=$parsed_inc_lvrg_gain_loss?> / <?=( $gain_loss >= 0 ? '+' : '' )?><?=$pretty_lvrg_gain_loss_percent?>%</span> (<?=( $asset_worth_inc_lvrg >= 0 ? '' : '-' )?><?=$ct['conf']['currency']['bitcoin_currency_markets'][ $ct['conf']['gen']['bitcoin_primary_currency_pair'] ]?><?=$parsed_asset_worth_inc_lvrg?>)</p>'
