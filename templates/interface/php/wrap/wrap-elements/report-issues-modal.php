	  ...
	  <div class="accordion-item">
         <h2 class="accordion-header" id="heading_<?=$accord_var?>">
           <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_<?=$accord_var?>" aria-expanded="true" aria-controls="#collapse_<?=$accord_var?>">
            ...
            Automatic Install / Upgrade For 'Server Edition' With Debian and RedHat based Operating Systems, On Home / Internal Network &nbsp; <span class='red'>(recommended way to PRIVATELY / CHEAPLY use this app)</span>
 ...
 SEE <a href='https://raw.githubusercontent.com/taoteh1221/Open_Crypto_Tracker/main/DOCUMENTATION-ETC/CONFIG-EXAMPLE.txt' target='_blank'>DOCUMENTATION-ETC/CONFIG-EXAMPLE.txt</a> FOR A FULL EXAMPLE OF THE DEFAULT CONFIGURATION (ESPECIALLY IF YOU MESS UP config.php, lol)
	...
	?>
	  ...
	  <div class="accordion-item">
	    ...
	    <br /><br />
	    If none of the above solutions work, your last resort (before contacting me for support) is to wipe out all data in your cache directory folder within the app. THIS WILL ERASE YOUR CHART DATA, AND ANY APP OR PLUGIN SETTINGS YOU CUSTOMIZED IN NORMAL / MEDIUM SECURITY MODE, SO YOU MAY WANT TO BE SURE YOU HAVE SAVED CONFIGURATION AND CHART BACKUPS FIRST. After your configuration and chart data is backed up, delete the folder named 'cache' in the main directory of this app. Reloading the app web page should re-create the cache folder, with new / clean cache files.
	    <br /><br />
...
<span class='blue'>10)</span> The PLUGIN CONFIG SETTING 'ui_location' IS OPTIONAL, to determine WHERE the plugin should run (nav menu [instant-open] tab, nav menu [dedicated] page, in the user area 'tools' / 'more stats' sections, etc...defaults to 'none' if not set).
<br /><br />

<pre class='rounded' style='display: inline-block; padding-top: 1em !important;'><code class='hide-x-scroll less' style='white-space: nowrap; width: auto; display: inline-block;'>$plug['conf'][$this_plug]['ui_location'] = 'tools'; // 'none', 'nav_menu_tab', 'nav_menu_page', 'tools', 'more_stats'</code></pre>
<br /><br /><br />
