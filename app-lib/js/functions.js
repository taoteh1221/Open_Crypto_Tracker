...
function set_admin_security(obj) {
		...
		if ( obj.value == "normal" || obj.value == "enhanced" ) {
	     var admin_sec_level_set = confirm("'Enhanced' and 'Normal' admin security modes are currently BETA (TEST) FEATURES, AND USING THEM MAY LEAD TO ISSUES UPDATING YOUR APP CONFIGURATION (editing from the PHP config files will be DISABLED).\n\nYou can RE-DISABLE these BETA features AFTER activating them (by setting the security mode back to 'High'), and you will be able to update your app configuration from the PHP config files again.");
		}
		else {
...
function set_admin_2fa(obj=false, verify=false) {

		if ( obj && obj.value == "on" ) {
	     var admin_2fa_set = confirm("2FA security requires the admin account to enter a SECONDARY \"one-time-password\" generated by Google Authenticator / Microsoft Authenticator / Authy / etc. This provides an additional layer of robust security to the admin area, further protecting against unauthorized access.");
		}
		else if ( obj && obj.value == "off" ) {
	     var admin_2fa_set = confirm("Turning off 2FA security IS *NOT* RECOMMENDED. Doing so will GREATLY REDUCE PROTECTION FROM HACKERS.\n\nIF YOU LOSE YOUR AUTHENTICATOR APP ACCOUNT, YOU MUST DELETE THE FILE \"cache/vars/admin_area_2fa.dat\" TO DISABLE 2FA.");
		}
		
		if ( verify == true ) {
		$("#toggle_admin_2fa").submit(); // Triggers iframe "reloading" sequence
		}
		else if ( admin_2fa_set ) {
