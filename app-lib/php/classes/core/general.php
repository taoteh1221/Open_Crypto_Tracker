...
var $pt_array1 = array();
   ...
   return $pt_var->strip_non_alpha($str[0]);
   ...
   $phone_number = $pt_var->strip_non_alpha($str[0]);
   
   $network_name = trim( strtolower($str[1]) ); // Force lowercase lookups for reliability / consistency
   
      // Set text domain
      if ( trim($phone_number) != '' && isset($pt_conf['mob_net_txt_gateways'][$network_name]) ) {
      return trim($phone_number) . '@' . trim($pt_conf['mob_net_txt_gateways'][$network_name]); // Return formatted texting email address
      }
      else {
      return false;
      }
   ...
   $domain = $address[1];
      ...
      return "No mail server records found for domain '" . $pt_var->obfuscate_str($domain) . "' [obfuscated]";
      }
      else {
      return "valid";
      }
   ...
   global $pt_conf, $possible_http_users, $http_runtime_user;
      ...
      if ( !is_dir($path) ) {
         ...
         $oldmask = umask(0);
         $result = mkdir($path, octdec($pt_conf['dev']['chmod_cache_dir']), true); // Recursively create whatever path depth desired if non-existent
         umask($oldmask);
         return $result;
         }
         else {
         return  mkdir($path, octdec($pt_conf['dev']['chmod_cache_dir']), true); // Recursively create whatever path depth desired if non-existent
         }
   ...
   global $pt_var;
      ...
      $subpath_array = explode("/", $subpath);
         ...
         $path = str_replace($subpath_array[0], $pt_var->obfuscate_str($subpath_array[0], 1), $path);
         $path = str_replace($subpath_array[1], $pt_var->obfuscate_str($subpath_array[1], 5), $path);
         }
         // Files directly in /secured/
         else {
         $path = str_replace($subpath, $pt_var->obfuscate_str($subpath, 5), $path);
         }
            
      //$path = str_replace('cache/secured', $pt_var->obfuscate_str('cache', 0) . '/' . $pt_var->obfuscate_str('secured', 0), $path);
   ...
   $url = preg_replace("/:\/\//i", ") ", $url);
      ...
      $url = str_replace($pt_conf['gen']['etherscan_key'], $pt_var->obfuscate_str($pt_conf['gen']['etherscan_key'], 2), $url);
      }
      // Telegram
      elseif ( preg_match("/telegram/i", $url) ) {
      $url = str_replace($pt_conf['comms']['telegram_bot_token'], $pt_var->obfuscate_str($pt_conf['comms']['telegram_bot_token'], 2), $url); 
      }
      // Defipulse
      elseif ( preg_match("/defipulse/i", $url) ) {
      $url = str_replace($pt_conf['gen']['defipulse_key'], $pt_var->obfuscate_str($pt_conf['gen']['defipulse_key'], 2), $url); 
      }
   ...
   $hostData = array_reverse($hostData);
      ...
      if ( array_search($hostData[1] . '.' . $hostData[0], $pt_conf['dev']['top_level_domain_map']) !== false ) {
      $host = $hostData[2] . '.' . $hostData[1] . '.' . $hostData[0];
      } 
      elseif ( array_search($hostData[0], $pt_conf['dev']['top_level_domain_map']) !== false ) {
   ...
   global $pt_conf;
       ...
       if ( mb_strlen($username, $pt_conf['dev']['charset_default']) < 4 ) {
       $error .= "requires 4 minimum characters; ";
       }
       
       if ( mb_strlen($username, $pt_conf['dev']['charset_default']) > 30 ) {
   ...
   function subarray_pt_conf_upgrade($cat_key, $conf_key, $skip_upgrading) {
   
   global $upgraded_pt_conf, $cached_pt_conf, $check_default_pt_conf, $default_pt_conf;
   ...
   global $pt_conf, $smtp;
      ...
      if ( $charset == null ) {
      $charset = $pt_conf['dev']['charset_default'];
      }
      
      
      // Fallback, if no From email set in app config
      if ( $this->valid_email($pt_conf['comms']['from_email']) == 'valid' ) {
      $from_email = $pt_conf['comms']['from_email'];
      }
      else {
      $temp_data = explode("||", $pt_conf['comms']['smtp_login']);
   ...
   $category = preg_replace("/_debug/i", "", $category);
      ...
      if ( $pt_conf['dev']['log_verb'] == 'normal' ) {
   ...
   $type = pathinfo($save_as, PATHINFO_EXTENSION);
      ...
      if ( $type == 'csv' ) {
      $content_type = 'Content-type: text/csv; charset=' . $pt_conf['dev']['charset_default'];
      }
      else {
   ...
   global $pt_conf;
      ...
      if ( ( $handle = fopen($file, "r") ) != false ) {
         ...
         $asset = strtoupper($data[0]);
            ...
            // ONLY importing if it exists in $pt_conf['assets']
            if ( is_array($pt_conf['assets'][$asset]) ) {
   ...
   // HTML
   $content = $pt_gen->txt_between_tags('a', $html);
   ...
   // XML
   $content2 = $pt_gen->txt_between_tags('description', $xml, 1);
   ...
   */
   ...
   $temp_smtp_email_server = explode(":", $pt_conf['comms']['smtp_server'] );
   ...
   $csv_row[1] = $pt_var->rem_num_format($csv_row[1]); // Remove any number formatting in held amount
   
   // Remove any number formatting in paid amount, default paid amount to null if not a valid positive number
   $csv_row[2] = ( $pt_var->rem_num_format($csv_row[2]) >= 0 ? $pt_var->rem_num_format($csv_row[2]) : null ); 
      
   // If leverage amount input is corrupt, default to 0 (ALSO simple auto-correct if negative)
   $csv_row[3] = ( $pt_var->whole_int($csv_row[3]) != false && $csv_row[3] >= 0 ? $csv_row[3] : 0 ); 
      
   // If leverage is ABOVE 'margin_leverage_max', default to 'margin_leverage_max'
   $csv_row[3] = ( $csv_row[3] <= $pt_conf['power']['margin_leverage_max'] ? $csv_row[3] : $pt_conf['power']['margin_leverage_max'] ); 
   ...
   $csv_row[5] = ( $pt_var->whole_int($csv_row[5]) != false && $csv_row[5] >= 1 ? $csv_row[5] : 1 ); 
   ...
   global $pt_conf;
       ...
       elseif ( mb_strlen($password, $pt_conf['dev']['charset_default']) < $min_length ) {
   ...
   function upgrade_cache_pt_conf() {
   
   global $upgraded_pt_conf, $cached_pt_conf, $check_default_pt_conf, $default_pt_conf;
   
   $upgraded_pt_conf = $cached_pt_conf;
   ...
   $skip_upgrading = array(
      ...
      elseif ( $check_default_pt_conf != md5(serialize($default_pt_conf)) ) {
         ...
         // Check for new variables, and add them
         foreach ( $default_pt_conf as $cat_key => $cat_val ) {
         ...
         // Check for depreciated variables, and remove them
         foreach ( $cached_pt_conf as $cached_cat_key => $cached_cat_val ) {
   ...
   $set_charset = $pt_conf['dev']['charset_default'];
   ...
   $words = explode(" ", $content);
      ...
      $scan_charset = ( mb_detect_encoding($scan_val, 'auto') != false ? mb_detect_encoding($scan_val, 'auto') : null );
         ...
         if ( isset($scan_charset) && !preg_match("/" . $pt_conf['dev']['charset_default'] . "/i", $scan_charset) && !preg_match("/ASCII/i", $scan_charset) ) {
         $set_charset = $pt_conf['dev']['charset_unicode'];
         }
   ...
   $result['length'] = mb_strlen($temp_converted, $set_charset); // Get character length AFTER trim() / BEFORE bin2hex() processing
      ...
      if ( $set_charset == $pt_conf['dev']['charset_unicode'] ) {
      ...
      $result = explode("||", fgets($fn) );
         ...
         $data['time'] .= trim($result[0]) . '000,';  // Zingchart wants 3 more zeros with unix time (milliseconds)
            ...
            elseif ( $asset_performance_chart ) {
               ...
               $percent_change = $pt_var->num_to_str($percent_change); 
               ...
               $data['spot'] .= $result[1] . ',';
               $data['volume'] .= round($result[2], $pt_conf['power']['chart_crypto_vol_dec']) . ',';
               }
   ...
   $pt_conf['comms']['from_email'] = str_replace("\r\n", "", $pt_conf['comms']['from_email']); // windows -> unix
   $pt_conf['comms']['from_email'] = str_replace("\r", "", $pt_conf['comms']['from_email']);   // remaining -> unix
   
   // Trim any (remaining) whitespace off ends
   $pt_conf['comms']['from_email'] = trim($pt_conf['comms']['from_email']);
   $to = trim($to);
  ...
  $cache_filename = preg_replace("/:/", "_", $cache_filename);
   ...
   global $runtime_mode, $app_version, $base_dir, $pt_var;
   ...
   $system['portfolio_cache'] = ( $pt_var->num_to_str($portfolio_cache) > 0 ? $portfolio_cache : 0 );
