...
var $oct_array1 = array();
   ...
   return $oct_var->strip_non_alpha($str[0]);
   ...
   global $oct_conf, $oct_var;
   
   $str = explode("||",$str);
   
   $phone_number = $oct_var->strip_non_alpha($str[0]);
   
   $network_name = trim( strtolower($str[1]) ); // Force lowercase lookups for reliability / consistency
   
      // Set text domain
      if ( trim($phone_number) != '' && isset($oct_conf['mob_net_txt_gateways'][$network_name]) ) {
      return trim($phone_number) . '@' . trim($oct_conf['mob_net_txt_gateways'][$network_name]); // Return formatted texting email address
      }
      else {
      return false;
      }
   ...
   $domain = $address[1];
      ...
      return "No mail server records found for domain '" . $oct_var->obfuscate_str($domain) . "' [obfuscated]";
      }
      else {
      return "valid";
      }
   ...
   global $base_url, $base_dir, $oct_app_id;
   ...
   global $oct_conf, $possible_http_users, $http_runtime_user;
      ...
      if ( !is_dir($path) ) {
         ...
         $oldmask = umask(0);
         $result = mkdir($path, octdec($oct_conf['dev']['chmod_cache_dir']), true); // Recursively create whatever path depth desired if non-existent
         umask($oldmask);
         return $result;
         }
         else {
         return  mkdir($path, octdec($oct_conf['dev']['chmod_cache_dir']), true); // Recursively create whatever path depth desired if non-existent
         }
   ...
   global $password_pepper;
      ...
      $password_pepper_hashed = hash_hmac("sha256", $password, $password_pepper);
         ...
         $this->log('conf_error', 'hash_hmac() returned false in the oct_gen->pepper_hashed_pass() function');
         return false;
         }
         else {
   ...
   global $oct_var;
      ...
      $subpath_array = explode("/", $subpath);
         ...
         $path = str_replace($subpath_array[0], $oct_var->obfuscate_str($subpath_array[0], 1), $path);
         $path = str_replace($subpath_array[1], $oct_var->obfuscate_str($subpath_array[1], 5), $path);
         }
         // Files directly in /secured/
         else {
         $path = str_replace($subpath, $oct_var->obfuscate_str($subpath, 5), $path);
         }
            
      //$path = str_replace('cache/secured', $oct_var->obfuscate_str('cache', 0) . '/' . $oct_var->obfuscate_str('secured', 0), $path);
   ...
   $url = preg_replace("/:\/\//i", ") ", $url);
      ...
      $url = str_replace($oct_conf['gen']['etherscan_key'], $oct_var->obfuscate_str($oct_conf['gen']['etherscan_key'], 2), $url);
      }
      // Telegram
      elseif ( preg_match("/telegram/i", $url) ) {
      $url = str_replace($oct_conf['comms']['telegram_bot_token'], $oct_var->obfuscate_str($oct_conf['comms']['telegram_bot_token'], 2), $url); 
      }
      // Defipulse
      elseif ( preg_match("/defipulse/i", $url) ) {
      $url = str_replace($oct_conf['gen']['defipulse_key'], $oct_var->obfuscate_str($oct_conf['gen']['defipulse_key'], 2), $url); 
      }
   ...
   $hostData = array_reverse($hostData);
      ...
      if ( array_search($hostData[1] . '.' . $hostData[0], $oct_conf['dev']['top_level_domain_map']) !== false ) {
      $host = $hostData[2] . '.' . $hostData[1] . '.' . $hostData[0];
      } 
      elseif ( array_search($hostData[0], $oct_conf['dev']['top_level_domain_map']) !== false ) {
   ...
   global $password_pepper, $stored_admin_login;
      ...
      $input_password_pepper_hashed = hash_hmac("sha256", $input_password, $password_pepper);
         ...
         $this->log('conf_error', 'hash_hmac() returned false in the oct_gen->check_pepper_hashed_pass() function');
         return false;
         }
         else {
   ...
   $result = array();
      ...
      if ( $mode == 'u' ) {
          ...
          $result['max_dec'] = $oct_conf['gen']['prim_currency_dec_max'];
          $result['min_dec'] = 2;
          }
   ...
   global $oct_conf;
       ...
       if ( mb_strlen($username, $oct_conf['dev']['charset_default']) < 4 ) {
       $error .= "requires 4 minimum characters; ";
       }
       
       if ( mb_strlen($username, $oct_conf['dev']['charset_default']) > 30 ) {
   ...
   function subarray_oct_conf_upgrade($cat_key, $conf_key, $skip_upgrading) {
   
   global $upgraded_oct_conf, $cached_oct_conf, $check_default_oct_conf, $default_oct_conf;
   
      // Check for new variables, and add them
      foreach ( $default_oct_conf[$cat_key][$conf_key] as $setting_key => $setting_val ) {
      
         if ( is_array($setting_val) ) {
         $this->log('conf_error', 'Sub-array depth to deep for app config upgrade parser');
         }
         elseif ( !in_array($setting_key, $skip_upgrading) && !isset($upgraded_oct_conf[$cat_key][$conf_key][$setting_key]) ) {
         	
         $upgraded_oct_conf[$cat_key][$conf_key][$setting_key] = $default_oct_conf[$cat_key][$conf_key][$setting_key];
         
         $this->log(
         			'conf_error',
         			'Outdated app config, upgraded parameter oct_conf[' . $cat_key . '][' . $conf_key . '][' . $setting_key . '] imported (default value: ' . $default_oct_conf[$cat_key][$conf_key][$setting_key] . ')'
         			);
      ...
      foreach ( $cached_oct_conf[$cat_key][$conf_key] as $setting_key => $setting_val ) {
      
         if ( is_array($setting_val) ) {
         $this->log('conf_error', 'Sub-array depth to deep for app config upgrade parser');
         }
         elseif ( !in_array($setting_key, $skip_upgrading) && !isset($default_oct_conf[$cat_key][$conf_key][$setting_key]) ) {
         	
         unset($upgraded_oct_conf[$cat_key][$conf_key][$setting_key]);
         
         $this->log(
         			'conf_error',
         			'Depreciated app config, parameter oct_conf[' . $cat_key . '][' . $conf_key . '][' . $setting_key . '] removed'
         			);
   ...
   global $oct_conf, $smtp;
   
      if ( $charset == null ) {
      $charset = $oct_conf['dev']['charset_default'];
      }
      
      
      // Fallback, if no From email set in app config
      if ( $this->valid_email($oct_conf['comms']['from_email']) == 'valid' ) {
      $from_email = $oct_conf['comms']['from_email'];
      }
      else {
      $temp_data = explode("||", $oct_conf['comms']['smtp_login']);
   ...
   global $runtime_mode, $oct_conf, $logs_array;
   ...
   $category = preg_replace("/_debug/i", "", $category);
      ...
      if ( $oct_conf['dev']['log_verb'] == 'normal' ) {
   ...
   $type = pathinfo($save_as, PATHINFO_EXTENSION);
      ...
      if ( $type == 'csv' ) {
      $content_type = 'Content-type: text/csv; charset=' . $oct_conf['dev']['charset_default'];
      }
      else {
   ...
   global $oct_conf;
      ...
      if ( ( $handle = fopen($file, "r") ) != false ) {
         ...
         $asset = strtoupper($data[0]);
            ...
            // ONLY importing if it exists in $oct_conf['assets']
            if ( is_array($oct_conf['assets'][$asset]) ) {
   ...
   // HTML
   $content = $oct_gen->txt_between_tags('a', $html);
   ...
   // XML
   $content2 = $oct_gen->txt_between_tags('description', $xml, 1);
   ...
   */
   ...
   global $app_version, $base_dir, $oct_conf;
   ...
   $temp_smtp_email_login = explode("||", $oct_conf['comms']['smtp_login'] );
   $temp_smtp_email_server = explode(":", $oct_conf['comms']['smtp_server'] );
   ...
   $vars['cfg_debug_mode'] = $oct_conf['dev']['debug']; // Open Crypto Tracker debug mode setting
   $vars['cfg_strict_ssl'] = $oct_conf['dev']['smtp_strict_ssl']; // Open Crypto Tracker strict SSL setting
   ...
   $csv_row[1] = $oct_var->rem_num_format($csv_row[1]); // Remove any number formatting in held amount
   
   // Remove any number formatting in paid amount, default paid amount to null if not a valid positive number
   $csv_row[2] = ( $oct_var->rem_num_format($csv_row[2]) >= 0 ? $oct_var->rem_num_format($csv_row[2]) : null ); 
      
   // If leverage amount input is corrupt, default to 0 (ALSO simple auto-correct if negative)
   $csv_row[3] = ( $oct_var->whole_int($csv_row[3]) != false && $csv_row[3] >= 0 ? $csv_row[3] : 0 ); 
      
   // If leverage is ABOVE 'margin_leverage_max', default to 'margin_leverage_max'
   $csv_row[3] = ( $csv_row[3] <= $oct_conf['power']['margin_leverage_max'] ? $csv_row[3] : $oct_conf['power']['margin_leverage_max'] ); 
   ...
   $csv_row[5] = ( $oct_var->whole_int($csv_row[5]) != false && $csv_row[5] >= 1 ? $csv_row[5] : 1 ); 
      
   $csv_row[6] = strtolower($csv_row[6]); // Pairing to lowercase
      
      
      // Pairing auto-correction (if invalid pairing)
      if ( $csv_row[6] == '' || !is_array($oct_conf['assets'][ $csv_row[0] ]['pairing'][ $csv_row[6] ]) ) {
         
      $csv_row[5] = 1; // We need to reset the market id to 1 (it's ALWAYS 1 OR GREATER), as the pairing was not found
      
      // First key in $oct_conf['assets'][ $csv_row[0] ]['pairing']
      reset($oct_conf['assets'][ $csv_row[0] ]['pairing']);
      $csv_row[6] = key($oct_conf['assets'][ $csv_row[0] ]['pairing']);
      
      }
      // Market ID auto-correction (if invalid market ID)
      elseif ( sizeof($oct_conf['assets'][ $csv_row[0] ]['pairing'][ $csv_row[6] ]) < $csv_row[5] ) {
   ...
   global $oct_conf;
       ...
       if ( $min_length == $max_length && mb_strlen($password, $oct_conf['dev']['charset_default']) != $min_length ) {
       $error .= "MUST BE EXACTLY ".$min_length." characters; ";
       }
       elseif ( mb_strlen($password, $oct_conf['dev']['charset_default']) < $min_length ) {
       $error .= "requires AT LEAST ".$min_length." characters; ";
       }
       elseif ( mb_strlen($password, $oct_conf['dev']['charset_default']) > $max_length ) {
   ...
   global $oct_conf, $oct_cache, $price_alert_fixed_reset_array, $default_btc_prim_currency_pairing;
   ...
   $email_msg = 'The following ' . $count . ' ' . strtoupper($default_btc_prim_currency_pairing) . ' price alert fixed resets (run every ' . $oct_conf['charts_alerts']['price_alert_fixed_reset'] . ' days) have been processed, with the latest spot price data: ' . $reset_list;
   
   $notifyme_msg = $email_msg . ' Timestamp is ' . $this->time_date_format($oct_conf['gen']['loc_time_offset'], 'pretty_time') . '.';
   ...
   function upgrade_cache_oct_conf() {
   
   global $upgraded_oct_conf, $cached_oct_conf, $check_default_oct_conf, $default_oct_conf;
   
   $upgraded_oct_conf = $cached_oct_conf;
   ...
   $skip_upgrading = array(
      ...
      elseif ( $check_default_oct_conf != md5(serialize($default_oct_conf)) ) {
         ...
         // Check for new variables, and add them
         foreach ( $default_oct_conf as $cat_key => $cat_val ) {
            ...
            foreach ( $cat_val as $conf_key => $conf_val ) {
               ...
               if ( !in_array($cat_key, $skip_upgrading) && !in_array($conf_key, $skip_upgrading) ) {
                  ...
                  $this->subarray_oct_conf_upgrade($cat_key, $conf_key, $skip_upgrading);
                  }
                  elseif ( !isset($upgraded_oct_conf[$cat_key][$conf_key]) ) {
                  	
                  $upgraded_oct_conf[$cat_key][$conf_key] = $default_oct_conf[$cat_key][$conf_key];
                  
                  $this->log(
                  			'conf_error',
                  			'Outdated app config, upgraded parameter $oct_conf[' . $cat_key . '][' . $conf_key . '] imported (default value: ' . $default_oct_conf[$cat_key][$conf_key] . ')'
                  			);
         ...
         // Check for depreciated variables, and remove them
         foreach ( $cached_oct_conf as $cached_cat_key => $cached_cat_val ) {
            ...
            foreach ( $cached_cat_val as $cached_conf_key => $cached_conf_val ) {
               ...
               if ( !in_array($cached_cat_key, $skip_upgrading) && !in_array($cached_conf_key, $skip_upgrading) ) {
                  ...
                  $this->subarray_oct_conf_upgrade($cached_cat_key, $cached_conf_key, $skip_upgrading);
                  }
                  elseif ( !isset($default_oct_conf[$cached_cat_key][$cached_conf_key]) ) {
                  	
                  unset($upgraded_oct_conf[$cached_cat_key][$cached_conf_key]);
                  
                  $this->log(
                  			'conf_error',
                  			'Depreciated app config parameter $oct_conf[' . $cached_cat_key . '][' . $cached_conf_key . '] removed'
                  			);
   ...
   $set_charset = $oct_conf['dev']['charset_default'];
   ...
   $words = explode(" ", $content);
      ...
      $scan_charset = ( mb_detect_encoding($scan_val, 'auto') != false ? mb_detect_encoding($scan_val, 'auto') : null );
         ...
         if ( isset($scan_charset) && !preg_match("/" . $oct_conf['dev']['charset_default'] . "/i", $scan_charset) && !preg_match("/ASCII/i", $scan_charset) ) {
         $set_charset = $oct_conf['dev']['charset_unicode'];
         }
   ...
   $result['length'] = mb_strlen($temp_converted, $set_charset); // Get character length AFTER trim() / BEFORE bin2hex() processing
      ...
      if ( $set_charset == $oct_conf['dev']['charset_unicode'] ) {
   ...
   global $oct_conf, $oct_var, $default_btc_prim_currency_pairing, $runtime_nonce, $runtime_data;
      ...
      if ( array_key_exists($chart_format, $oct_conf['power']['btc_currency_markets']) && !array_key_exists($chart_format, $oct_conf['power']['crypto_pairing']) ) {
      ...
      $result = explode("||", fgets($fn) );
         ...
         $data['time'] .= trim($result[0]) . '000,';  // Zingchart wants 3 more zeros with unix time (milliseconds)
            ...
            elseif ( $asset_performance_chart ) {
               ...
               $percent_change = $oct_var->num_to_str($percent_change); 
               ...
               if ( $fiat_formatting ) {
               $data['spot'] .= ( $oct_var->num_to_str($result[1]) >= 1 ? number_format((float)$result[1], 2, '.', '')  :  round($result[1], $oct_conf['gen']['prim_currency_dec_max'])  ) . ',';
               $data['volume'] .= round($result[2]) . ',';
               }
               // Non-stablecoin crypto
               else {
               $data['spot'] .= $result[1] . ',';
               $data['volume'] .= round($result[2], $oct_conf['power']['chart_crypto_vol_dec']) . ',';
               }
   ...
   $oct_conf['comms']['from_email'] = str_replace("\r\n", "", $oct_conf['comms']['from_email']); // windows -> unix
   $oct_conf['comms']['from_email'] = str_replace("\r", "", $oct_conf['comms']['from_email']);   // remaining -> unix
   
   // Trim any (remaining) whitespace off ends
   $oct_conf['comms']['from_email'] = trim($oct_conf['comms']['from_email']);
   $to = trim($to);
      ...
      return @$this->smtp_mail($to, $subj, $msg, $content_type, $charset); 
         ...
         if ( PHP_VERSION_ID >= 70200 ) {
            ...
            $headers = array(
                        'From' => $oct_conf['comms']['from_email'],
            ...
            $headers = 'From: ' . $oct_conf['comms']['from_email'] . "\r\n" .
            'X-Mailer: Open_Crypto_Tracker/' . $app_version . ' - PHP/' . phpversion() . "\r\n" .
            'Content-Type: ' . $content_type . '/plain; charset=' . $charset;
  ...
  global $base_dir, $oct_conf, $oct_cache, $runtime_mode, $proxies_checked;
  ...
  $cache_filename = preg_replace("/:/", "_", $cache_filename);
      ...
      if ( $oct_conf['comms']['proxy_alert_runtime'] == 'all' ) {
      $run_alerts = 1;
      }
      elseif ( $oct_conf['comms']['proxy_alert_runtime'] == 'cron' && $runtime_mode == 'cron' ) {
      $run_alerts = 1;
      }
      elseif ( $oct_conf['comms']['proxy_alert_runtime'] == 'ui' && $runtime_mode == 'ui' ) {
      $run_alerts = 1;
      }
      else {
      $run_alerts = null;
      ...
      if ( $run_alerts == 1 && $oct_cache->update_cache('cache/alerts/proxy-check-'.$cache_filename.'.dat', ( $oct_conf['comms']['proxy_alert_freq_max'] * 60 ) ) == true
      ...
      $email_alert = " The proxy " . $problem_proxy . " recently did not receive data when accessing this endpoint: \n " . $obfusc_url_data . " \n \n A check on this proxy was performed at " . $proxy_test_url . ", and results logged: \n ============================================================== \n " . $cached_logs . " \n ============================================================== \n \n ";
         ...
         // Send out alerts
         if ( $misconfigured == 1 || $oct_conf['comms']['proxy_alert_checkup_ok'] == 'include' ) {
                           
                           
             // Message parameter added for desired comm methods (leave any comm method blank to skip sending via that method)
             if ( $oct_conf['comms']['proxy_alert'] == 'all' ) {
             ...
             elseif ( $oct_conf['comms']['proxy_alert'] == 'email' ) {
             ...
             elseif ( $oct_conf['comms']['proxy_alert'] == 'text' ) {
             ...
             elseif ( $oct_conf['comms']['proxy_alert'] == 'notifyme' ) {
                  $send_params['notifyme'] = $notifyme_alert;
             }
             elseif ( $oct_conf['comms']['proxy_alert'] == 'telegram' ) {
   ...
   global $runtime_mode, $app_version, $base_dir, $oct_var;
   ...
   $system['portfolio_cache'] = ( $oct_var->num_to_str($portfolio_cache) > 0 ? $portfolio_cache : 0 );
