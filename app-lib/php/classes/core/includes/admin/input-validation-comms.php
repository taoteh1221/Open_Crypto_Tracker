...
$ct['update_config_error'] = 'SMTP Login formatting is NOT valid (MUST be: username||password)';
}
elseif ( isset($_POST['comms']['smtp_server']) && $_POST['comms']['smtp_server'] != '' && sizeof($smtp_server_check) < 2 ) {
$ct['update_config_error'] = 'SMTP Server formatting is NOT valid (MUST be: domain_or_ip:port_number)';
}
// Mobile text check
elseif ( isset($_POST['comms']['to_mobile_text']) && $_POST['comms']['to_mobile_text'] != '' && sizeof($to_mobile_text_check) < 2 ) {
$ct['update_config_error'] = 'To Mobile Text formatting is NOT valid (MUST be: mobile_number||network_name)';
}
