...
var $ocpt_array1 = array();
   ...
   // Globals
   global $_POST, $base_dir, $ocpt_var, $ocpt_api, $btc_worth_array, $coin_stats_array, $td_color_zebra, $cap_data_force_usd, $theme_selected, $prim_curr_market_standalone, $ocpt_conf, $selected_btc_prim_curr_value, $alert_percent, $show_secondary_trade_value, $coingecko_api, $coinmarketcap_api;
   ...
   $original_market = $selected_exchange;
     ...
     if ( $ocpt_var->num_to_str($asset_amount) > 0.00000000 || strtolower($asset_name) == 'bitcoin' ) {
       ...
       foreach ( $all_pairing_markets as $key => $value ) {
           ...
           $selected_exchange = $key;
            ...
            $ocpt_conf['general']['btc_prim_exchange'] = $key;
            $ocpt_conf['general']['btc_prim_curr_pairing'] = $selected_pairing;
                   ...
                   $ocpt_conf['assets']['MISCASSETS']['name'] = 'Misc. '.strtoupper($selected_pairing).' Value';
                   }
       
            ?>
            
            <script>
            window.btc_prim_curr_value = '<?=$ocpt_api->market('BTC', $key, $ocpt_conf['assets']['BTC']['pairing'][$selected_pairing][$key])['last_trade']?>';
            
            window.btc_prim_curr_pairing = '<?=strtoupper($selected_pairing)?>';
            </script>
     ...
     $selected_btc_pairing_id = $ocpt_conf['assets']['BTC']['pairing'][$ocpt_conf['general']['btc_prim_curr_pairing']][$ocpt_conf['general']['btc_prim_exchange']];
     $selected_btc_prim_curr_value = $ocpt_api->market('BTC', $ocpt_conf['general']['btc_prim_exchange'], $selected_btc_pairing_id)['last_trade'];
       
       
       // Log any Bitcoin market errors
       if ( !isset($selected_btc_prim_curr_value) || $selected_btc_prim_curr_value == 0 ) {
       app_logging('market_error', 'ocpt_asset->ui_coin_row() Bitcoin primary currency value not properly set', 'exchange: ' . $ocpt_conf['general']['btc_prim_exchange'] . '; pairing_id: ' . $selected_btc_pairing_id . '; value: ' . $selected_btc_prim_curr_value );
       }
     ...
     $all_pairings = $ocpt_conf['assets'][$asset_symbol]['pairing'];
      ...
      if ( $ocpt_conf['general']['prim_mcap_site'] == 'coingecko' && sizeof($coingecko_api) < 1 ) {
      $coingecko_api = $ocpt_api->coingecko();
      }
      elseif ( $ocpt_conf['general']['prim_mcap_site'] == 'coinmarketcap' && sizeof($coinmarketcap_api) < 1 ) {
     ...
     // Consolidate function calls for runtime speed improvement
     $asset_market_data = $ocpt_api->market($asset_symbol, $selected_exchange, $market_id, $selected_pairing);
     ...
     $coin_value_total_raw = $ocpt_var->num_to_str($asset_amount * $coin_value_raw);
     ...
     $pairing_btc_value = pairing_btc_value($selected_pairing); 
      ...
      if ( $pairing_btc_value == null ) {
      app_logging('market_error', 'pairing_btc_value(\''.$selected_pairing.'\') returned null in pt_asset->ui_coin_row(), likely from exchange API request failure');
      }
     
     
     $coin_prim_curr_worth_raw = ($coin_value_total_raw * $pairing_btc_value) * $selected_btc_prim_curr_value;
       ...
       $btc_worth_array[$asset_symbol] = $asset_amount;
        ...
        $btc_worth_array[$asset_symbol] = $ocpt_var->num_to_str($coin_value_total_raw * $pairing_btc_value);
        }
       ...
       if ( array_key_exists($selected_pairing, $ocpt_conf['power_user']['btc_currency_markets']) && !array_key_exists($selected_pairing, $ocpt_conf['power_user']['crypto_pairing']) ) {
      $fiat_eqiv = 1;
       }
     ...
     // Get trade volume
     $trade_volume = $asset_market_data['24hr_prim_curr_volume'];
   ...
   // Globals
   global $base_dir, $ocpt_cache, $ocpt_var, $ocpt_api, $ocpt_conf, $default_btc_prim_exchange, $default_btc_prim_curr_value, $default_btc_prim_curr_pairing, $price_alert_fixed_reset_array;
   ...
   $asset = strtoupper($asset);
     ...
     // #FOR CLEAN CODE#, RUN CHECK TO MAKE SURE IT'S NOT A CRYPTO AS WELL...WE HAVE A COUPLE SUPPORTED, BUT WE ONLY WANT DESIGNATED FIAT-EQIV HERE
     if ( array_key_exists($pairing, $ocpt_conf['power_user']['btc_currency_markets']) && !array_key_exists($pairing, $ocpt_conf['power_user']['crypto_pairing']) ) {
     $fiat_eqiv = 1;
     }
   /////////////////////////////////////////////////////////////////
   ...
   $asset_market_data = $ocpt_api->market($asset, $exchange, $ocpt_conf['assets'][$asset]['pairing'][$pairing][$exchange], $pairing);
     ...
     $asset_prim_curr_value_raw = $asset_market_data['last_trade']; 
     }
     // BTC PAIRINGS CONVERTED TO PRIMARY CURRENCY CONFIG (EQUIV) CHARTS
     elseif ( $pairing == 'btc' ) {
     $asset_prim_curr_value_raw = number_format( $default_btc_prim_curr_value * $asset_market_data['last_trade'] , 8, '.', '');
     }
     ...
     $pairing_btc_value = pairing_btc_value($pairing); 
       ...
       if ( $pairing_btc_value == null ) {
       app_logging('market_error', 'pairing_btc_value() returned null in pt_asset->charts_price_alerts()', 'pairing: ' . $pairing);
       }
     
     $asset_prim_curr_value_raw = number_format( $default_btc_prim_curr_value * ( $asset_market_data['last_trade'] * $pairing_btc_value ) , 8, '.', '');
   ...
   /////////////////////////////////////////////////////////////////
   $volume_pairing_raw = $ocpt_var->num_to_str($asset_market_data['24hr_pairing_volume']); // If available, we'll use this for chart volume UX
   $volume_prim_curr_raw = $asset_market_data['24hr_prim_curr_volume'];
   ...
   /////////////////////////////////////////////////////////////////
     ...
     // Return false if we have no $default_btc_prim_curr_value
     ...
     if ( $ocpt_var->num_to_str( trim($asset_prim_curr_value_raw) ) >= 0.00000001 ) {
     // Continue
     }
     else {
   ...
   // Round PAIRING volume to only keep $ocpt_conf['power_user']['charts_crypto_volume_dec'] decimals max (for crypto volume etc), to save on data set / storage size
   $volume_pairing_raw = ( isset($volume_pairing_raw) ? round($volume_pairing_raw, ( $fiat_eqiv == 1 ? 0 : $ocpt_conf['power_user']['charts_crypto_volume_dec'] ) ) : null );	
     
     
   // Round PRIMARY CURRENCY CONFIG asset price to only keep $ocpt_conf['general']['prim_curr_dec_max'] decimals maximum 
   // (or only 2 decimals if worth $ocpt_conf['general']['prim_curr_dec_max_thres'] or more), to save on data set / storage size
   $asset_prim_curr_value_raw = ( $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= $ocpt_conf['general']['prim_curr_dec_max_thres'] ? round($asset_prim_curr_value_raw, 2) : round($asset_prim_curr_value_raw, $ocpt_conf['general']['prim_curr_dec_max']) );
     
     
     // If fiat equivalent format, round asset price 
     // to only keep $ocpt_conf['general']['prim_curr_dec_max'] decimals maximum 
     // (or only 2 decimals if worth $ocpt_conf['general']['prim_curr_dec_max_thres'] or more), to save on data set / storage size
      if ( $fiat_eqiv == 1 ) {
      $asset_pairing_value_raw = ( $ocpt_var->num_to_str($asset_pairing_value_raw) >= $ocpt_conf['general']['prim_curr_dec_max_thres'] ? round($asset_pairing_value_raw, 2) : round($asset_pairing_value_raw, $ocpt_conf['general']['prim_curr_dec_max']) );
      }
   ...
   /////////////////////////////////////////////////////////////////
     ...
     if ( $mode == 'both' && $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= 0.00000001 && $ocpt_conf['general']['asset_charts_toggle'] == 'on'
     || $mode == 'chart' && $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= 0.00000001 && $ocpt_conf['general']['asset_charts_toggle'] == 'on' ) {
     ...
     $prim_curr_chart_data = $now . '||' . $asset_prim_curr_value_raw . '||' . $volume_prim_curr_raw;
     $ocpt_cache->save_file($prim_curr_chart_path, $prim_curr_chart_data . "\n", "append", false);  // WITH newline (UNLOCKED file write)
       ...
       $ocpt_cache->save_file($crypto_secondary_currency_chart_path, $crypto_secondary_currency_chart_data . "\n", "append", false); // WITH newline (UNLOCKED file write)
       }
     ...
     // Grab any cached price alert data
      ...
      $cached_array = explode("||", $data_file);
         ...
         $cached_asset_prim_curr_value = $data_file;
         $cached_prim_curr_volume = -1;
         $cached_pairing_volume = -1;
         }
         else {
         $cached_asset_prim_curr_value = $cached_array[0];  // PRIMARY CURRENCY CONFIG token value
         $cached_prim_curr_volume = round($cached_array[1]); // PRIMARY CURRENCY CONFIG volume value (round PRIMARY CURRENCY CONFIG volume to nullify insignificant decimal amounts skewing checks)
         ...
         if ( $ocpt_var->num_to_str( trim($cached_asset_prim_curr_value) ) >= 0.00000001 && $ocpt_var->num_to_str( trim($asset_prim_curr_value_raw) ) >= 0.00000001 ) {
         
         
         // PRIMARY CURRENCY CONFIG price percent change (!MUST BE! absolute value)
         $percent_change = abs( ($asset_prim_curr_value_raw - $cached_asset_prim_curr_value) / abs($cached_asset_prim_curr_value) * 100 );
       ...
       $exchange_text = snake_case_to_name($exchange);
           ...
           // UX / UI variables
           if ( $ocpt_var->num_to_str($asset_prim_curr_value_raw) < $ocpt_var->num_to_str($cached_asset_prim_curr_value) ) {
           $change_symbol = '-';
           $increase_decrease = 'decreased';
           }
           elseif ( $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= $ocpt_var->num_to_str($cached_asset_prim_curr_value) ) {
           $change_symbol = '+';
         ...
         $volume_percent_change = $ocpt_var->num_to_str($volume_percent_change); // Better decimal support
                 ...
                 if ( $cached_prim_curr_volume <= 0 && $volume_prim_curr_raw <= 0 ) { // ONLY PRIMARY CURRENCY CONFIG VOLUME CALCULATION RETURNS -1 ON EXCHANGE VOLUME ERROR
                 ...
                 elseif ( $cached_prim_curr_volume <= 0 && $volume_pairing_raw >= $cached_pairing_volume ) { // ONLY PRIMARY CURRENCY CONFIG VOLUME CALCULATION RETURNS -1 ON EXCHANGE VOLUME ERROR
                 ...
                 elseif ( $cached_prim_curr_volume > 0 && $volume_pairing_raw < $cached_pairing_volume ) {
                 $volume_change_symbol = '-';
                 }
                 elseif ( $cached_prim_curr_volume > 0 && $volume_pairing_raw > $cached_pairing_volume ) {
                 $volume_change_symbol = '+';
                 ...
                 $whale_min_volume_percent_increase_24hr_average = $ocpt_var->num_to_str( trim($whale_alert_threshold[2]) );
                 
                 $whale_min_volume_currency_increase_24hr_average = $ocpt_var->num_to_str( trim($whale_alert_threshold[3]) );
                   ...
                   && $ocpt_var->num_to_str($volume_change_symbol . $volume_percent_change / $last_cached_days) >= $whale_min_volume_percent_increase_24hr_average 
                   && $ocpt_var->num_to_str( ($volume_prim_curr_raw - $cached_prim_curr_volume) / $last_cached_days ) >= $whale_min_volume_currency_increase_24hr_average ) {
                   $whale_alert = 1;
                   }
                 ...
                 if ( $volume_prim_curr_raw >= 0 && $volume_prim_curr_raw < $ocpt_conf['comms']['price_alert_min_vol'] ) {
                 $send_alert = null;
                 }
                 ...
                 if ( $volume_prim_curr_raw == -1 && $ocpt_conf['comms']['price_alert_block_vol_error'] == 'on' ) {
                 $send_alert = null;
                 }
                 ...
                 $desc_alert_type = ( $ocpt_conf['charts_alerts']['price_alert_fixed_reset'] > 0 ? 'reset' : 'alert' );
                   ...
                   if ( $cached_prim_curr_volume >= 0 && $cached_prim_curr_volume <= 1 ) {
                   $volume_describe = strtoupper($default_btc_prim_curr_pairing) . ' volume was ' . $ocpt_conf['power_user']['btc_currency_markets'][$default_btc_prim_curr_pairing] . $cached_prim_curr_volume . ' last price ' . $desc_alert_type . ', and ';
                   $volume_describe_mobile = strtoupper($default_btc_prim_curr_pairing) . ' volume up from ' . $ocpt_conf['power_user']['btc_currency_markets'][$default_btc_prim_curr_pairing] . $cached_prim_curr_volume . ' last ' . $desc_alert_type;
                   }
                   // Best we can do feasibly for UX on volume reporting errors
                   elseif ( $cached_prim_curr_volume == -1 ) { // ONLY PRIMARY CURRENCY CONFIG VOLUME CALCULATION RETURNS -1 ON EXCHANGE VOLUME ERROR
                   $volume_describe = strtoupper($default_btc_prim_curr_pairing) . ' volume was NULL last price ' . $desc_alert_type . ', and ';
                   $volume_describe_mobile = strtoupper($default_btc_prim_curr_pairing) . ' volume up from NULL last ' . $desc_alert_type;
                   }
                   else {
                 ...
                 $asset_prim_curr_text = ( $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= $ocpt_conf['general']['prim_curr_dec_max_thres'] ? $ocpt_var->num_pretty($asset_prim_curr_value_raw, 2) : $ocpt_var->num_pretty($asset_prim_curr_value_raw, $ocpt_conf['general']['prim_curr_dec_max']) );
                       
                 $volume_prim_curr_text = $ocpt_conf['power_user']['btc_currency_markets'][$default_btc_prim_curr_pairing] . number_format($volume_prim_curr_raw, 0, '.', ',');
                       
                 $volume_change_text = 'has ' . ( $volume_change_symbol == '+' ? 'increased ' : 'decreased ' ) . $volume_change_symbol . number_format($volume_percent_change, 2, '.', ',') . '% to a ' . strtoupper($default_btc_prim_curr_pairing) . ' value of';
                 ...
                 $volume_change_text_mobile = '(' . $volume_change_symbol . number_format($volume_percent_change, 2, '.', ',') . '% ' . $volume_describe_mobile . ')';
                   ...
                   // ONLY PRIMARY CURRENCY CONFIG VOLUME CALCULATION RETURNS -1 ON EXCHANGE VOLUME ERROR
                   if ( $cached_prim_curr_volume == -1 || $volume_prim_curr_raw == -1 ) {
                   ...
                   if ( $volume_prim_curr_raw == null && $ocpt_conf['comms']['price_alert_min_vol'] > 0 || $volume_prim_curr_raw < 1 && $ocpt_conf['comms']['price_alert_min_vol'] > 0 ) {
                   ...
                   // Successfully received > 0 volume data, at or above an enabled volume filter
                       if ( $volume_prim_curr_raw > 0 && $ocpt_conf['comms']['price_alert_min_vol'] > 0 && $volume_prim_curr_raw >= $ocpt_conf['comms']['price_alert_min_vol'] ) {
                   $email_volume_summary = '24 hour ' . $volume_describe . $volume_change_text . ' ' . $volume_prim_curr_text . ' (volume filter on).';
                   }
                   ...
                   elseif ( $volume_prim_curr_raw >= 0 ) {
                   $email_volume_summary = '24 hour ' . $volume_describe . $volume_change_text . ' ' . $volume_prim_curr_text . ( $volume_prim_curr_raw == 0 ? ' (probable volume discrepancy detected' . $volume_filter_skipped_text . ')' : '' ) . '.'; 
                   }
                 ...
                 $email_message = ( $whale_alert == 1 ? 'WHALE ALERT: ' : '' ) . 'The ' . $asset . ' trade value in the ' . strtoupper($pairing) . ' market at the ' . $exchange_text . ' exchange has ' . $increase_decrease . ' ' . $change_symbol . $percent_change_text . '% in ' . strtoupper($default_btc_prim_curr_pairing) . ' value to ' . $ocpt_conf['power_user']['btc_currency_markets'][$default_btc_prim_curr_pairing] . $asset_prim_curr_text . ' over the past ' . $last_cached_time . ' since the last price ' . $desc_alert_type . '. ' . $email_volume_summary;
                       
                 // Were're just adding a human-readable timestamp to smart home (audio) alerts
                 $notifyme_message = $email_message . ' Timestamp: ' . time_date_format($ocpt_conf['general']['local_time_offset'], 'pretty_time') . '.';
                       
                 $text_message = ( $whale_alert == 1 ? '🐳 ' : '' ) . $asset . ' / ' . strtoupper($pairing) . ' @ ' . $exchange_text . ' ' . $increase_decrease . ' ' . $change_symbol . $percent_change_text . '% in ' . strtoupper($default_btc_prim_curr_pairing) . ' value to ' . $ocpt_conf['power_user']['btc_currency_markets'][$default_btc_prim_curr_pairing] . $asset_prim_curr_text . ' over ' . $last_cached_time . '. 24 Hour ' . strtoupper($default_btc_prim_curr_pairing) . ' Volume: ' . $volume_prim_curr_text . ' ' . $volume_change_text_mobile;
       ...
       if ( $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= 0.00000001 && !file_exists('cache/alerts/'.$asset_data.'.dat') ) {
       $ocpt_cache->save_file($base_dir . '/cache/alerts/'.$asset_data.'.dat', $alert_cache_contents); 
       }
       elseif ( $send_alert != 1 && $ocpt_conf['charts_alerts']['price_alert_fixed_reset'] >= 1 && $ocpt_var->num_to_str($asset_prim_curr_value_raw) >= 0.00000001 
