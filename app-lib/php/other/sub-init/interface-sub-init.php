...
$pt_conf['gen']['prim_mcap_site'] = ( $sel_opt['alert_percent'][0] != '' ? $sel_opt['alert_percent'][0] : $pt_conf['gen']['prim_mcap_site'] );
...
$sel_opt['show_secondary_trade_val'] = ( $_POST['show_secondary_trade_val'] != '' ? $_POST['show_secondary_trade_val'] : $_COOKIE['show_secondary_trade_val'] );
	...
	if ( !array_key_exists($sel_opt['show_secondary_trade_val'], $pt_conf['power']['crypto_pairing']) ) {
...
///////////////////////////////////////////////////////////////////////
	...
	$sel_opt['show_charts'] = explode(',', rtrim( ( $_POST['show_charts'] != '' ? $_POST['show_charts'] : $_COOKIE['show_charts'] ) , ',') );
		...
		foreach ($scan_charts as $market_key) {
			...
			if ( array_key_exists($market_key, $pt_conf['charts_alerts']['tracked_markets']) && stristr($sel_opt['show_charts'][$loop], '_') ) {
				
			$chart_params = explode('_', $pt_var->strip_brackets($sel_opt['show_charts'][$loop]) );
...
$sel_opt['sorted_asc_desc'] = $sort_array[1];
	...
	$sel_opt['theme_selected'] = $pt_conf['gen']['default_theme'];
	}
