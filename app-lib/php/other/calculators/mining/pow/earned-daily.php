...
?>				
				...
				<?php
				$val_per_asset = round( $mined_asset_val * $sel_btc_prim_currency_val , 8);
				
				$val_per_asset = ( $pt_var->num_to_str($val_per_asset) >= $pt_conf['gen']['prim_currency_dec_max_thres'] ? round($val_per_asset, 2) : round($val_per_asset, $pt_conf['gen']['prim_currency_dec_max']) );
				
				echo ( $pow_asset_data['symbol'] == 'btc' ? number_format($sel_btc_prim_currency_val, 2) . ' ' . strtoupper($pt_conf['gen']['btc_prim_currency_pairing']) : number_format($mined_asset_val, 8) . ' BTC (' . $pt_conf['power']['btc_currency_markets'][$pt_conf['gen']['btc_prim_currency_pairing']] . $val_per_asset . ' '.strtoupper($pt_conf['gen']['btc_prim_currency_pairing']).')' );
				?>
