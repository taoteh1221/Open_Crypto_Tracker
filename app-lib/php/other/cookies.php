...
$ct_gen->store_cookie("notes", $_POST['notes'], mktime()+31536000);
header("Location: " . $ct_gen->start_page($_GET['start_page']));
exit;
}
elseif ( $_POST['update_notes'] == 1 && trim($_POST['notes']) == '' && $_COOKIE['notes'] ) {
$ct_gen->store_cookie("notes", " ", mktime()+31536000); // Initialized with some whitespace when blank
header("Location: " . $ct_gen->start_page($_GET['start_page']));
exit;
}
...
$ct_gen->delete_all_cookies(); // Delete any existing cookies, if cookies have been disabled
}
elseif ( $_POST['submit_check'] == 1 && $_POST['use_cookies'] == 1 || $run_csv_import == 1 && $_COOKIE['coin_amounts'] != '' ) {
 ...
 if ( $_POST['submit_check'] == 1 ) {
   ...
   foreach ( $_POST as $key => $val ) {
      ...
      $_POST[$key] = $ct_var->strip_formatting($val);
