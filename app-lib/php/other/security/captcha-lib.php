...
for($i = 0; $i < round($app_config['developer']['captcha_text_size'] * 3.15); $i++) {
...
for ( $i = 0; $i < round($app_config['developer']['captcha_text_size'] / 1.35); $i++ ) {
...
$letter_space = round( ( $app_config['developer']['captcha_image_width'] - ($app_config['developer']['captcha_text_margin'] * 2) ) / $app_config['developer']['captcha_chars_length'] ) + 2;
$initial = rand($app_config['developer']['captcha_text_margin'], ($app_config['developer']['captcha_text_margin'] * 2) ) + $app_config['developer']['captcha_text_margin'] + 4;

$angle = random_int( (0 - $app_config['power_user']['captcha_text_angle']) , $app_config['power_user']['captcha_text_angle']);
   
imagettftext($image, $app_config['developer']['captcha_text_size'], $angle, $initial + round($i * $letter_space), rand( ($app_config['developer']['captcha_text_size'] + ($app_config['developer']['captcha_text_margin'] * 4) ), ($app_config['developer']['captcha_image_height'] - ($app_config['developer']['captcha_text_margin'] * 5) ) ), $textcolors[rand(0, 1)], $fonts[array_rand($fonts)], $captcha_string[$i]);
