 ...
 */
	...
	////////////////////////////////////////////////////////////
	if ( isset($oct_conf['comms']['upgrade_alert']) && $oct_conf['comms']['upgrade_alert'] != 'off' && $oct_cache->update_cache($base_dir . '/cache/vars/upgrade_check_latest_version.dat', 1440) == true ) {
	
	
	$upgrade_check_jsondata = @$oct_cache->ext_data('url', 'https://api.github.com/repos/taoteh1221/Open_Crypto_Tracker/releases/latest', 0); // Don't cache API data
	
	$upgrade_check_data = json_decode($upgrade_check_jsondata, true);
	
	$upgrade_check_latest_version = trim($upgrade_check_data["tag_name"]);
	
	$oct_cache->save_file($base_dir . '/cache/vars/upgrade_check_latest_version.dat', $upgrade_check_latest_version);
	...
	$app_bug_fixes = $app_version_array[2];
		...
		// EVENTUALLY PUT UI ALERT LOGIC HERE
			...
			// Email / text / alexa notification reminders (if it's been $oct_conf['comms']['upgrade_alert_reminder'] days since any previous reminder)
			if ( $oct_cache->update_cache($base_dir . '/cache/events/upgrade_check_reminder.dat', ( $oct_conf['comms']['upgrade_alert_reminder'] * 1440 ) ) == true ) {
			...
			$email_notifyme_msg = $upgrade_check_msg . ' (you have upgrade reminders triggered every '.$oct_conf['comms']['upgrade_alert_reminder'].' days in the configuration settings)';
			...
			$email_only_with_upgrade_command = $email_notifyme_msg . "\n\n" . 'Quick / easy upgrading can be done by copying / pasting / running this command, using the "Terminal" app in your Ubuntu / Raspberry Pi system menu, or logging in remotely from another device via SSH (user must have sudo privileges):' . "\n\n" . 'wget --no-cache -O FOLIO-INSTALL.bash https://git.io/JoDFD;chmod +x FOLIO-INSTALL.bash;sudo ./FOLIO-INSTALL.bash';
					...
					if ( $oct_conf['comms']['upgrade_alert'] == 'all' ) {
					...
					$encoded_text_alert = $oct_gen->charset_encode($upgrade_check_msg); // Unicode support included for text messages (emojis / asian characters / etc )
					...
					elseif ( $oct_conf['comms']['upgrade_alert'] == 'email' ) {
					...
					elseif ( $oct_conf['comms']['upgrade_alert'] == 'text' ) {
					...
					elseif ( $oct_conf['comms']['upgrade_alert'] == 'notifyme' ) {
					$upgrade_check_send_params['notifyme'] = $email_notifyme_msg;
					}
					elseif ( $oct_conf['comms']['upgrade_alert'] == 'telegram' ) {
					$upgrade_check_send_params['telegram'] = $email_only_with_upgrade_command;
					}
					elseif ( $oct_conf['comms']['upgrade_alert'] == 'ui' ) {
			...
			// Send notifications
			@$oct_cache->queue_notify($upgrade_check_send_params);
			
			// Track upgrade check reminder event occurrence			
			$oct_cache->save_file($base_dir . '/cache/events/upgrade_check_reminder.dat', $oct_gen->time_date_format(false, 'pretty_date_time') );
