			...
			foreach ( $asset_val['pairing'] as $pairing_key => $pairing_val ) {
				...
				foreach ( $pairing_val as $key => $val ) {
					...
					$markets_test_data = $pt_api->market( strtoupper($asset_key) , $key, $val, $pairing_key);
						...
						$pt_gen->app_logging('market_error', 'No market price data available for ' . strtoupper($asset_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $pt_gen->key_to_name($key) );
						}
					
						if ( $markets_test_data['24hr_prim_currency_vol'] == NULL || $markets_test_data['24hr_prim_currency_vol'] < 1 ) {
						$pt_gen->app_logging('market_error', 'No market volume data available for ' . strtoupper($asset_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $pt_gen->key_to_name($key) );
						}
