...
if ( $runtime_mode == 'ui' ) {
	...
	if ( $ct_conf['dev']['debug'] == 'all' || $ct_conf['dev']['debug'] == 'alerts_charts' ) {
		...
		$check_pairing_name = $ct_conf['assets'][$check_asset]['pairing'][ $check_asset_params[1] ][ $check_asset_params[0] ];
		
		// Consolidate function calls for runtime speed improvement
		$charts_test_data = $ct_api->market($check_asset, $check_asset_params[0], $check_pairing_name, $check_asset_params[1]);
			...
			foreach ( $asset_val['pairing'] as $pairing_key => $pairing_val ) {
				...
				foreach ( $pairing_val as $key => $val ) {
					...
					$markets_test_data = $ct_api->market( strtoupper($asset_key) , $key, $val, $pairing_key);
						...
						$ct_gen->log(
													'market_error',
													'No market price data available for ' . strtoupper($asset_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $ct_gen->key_to_name($key)
													);
						...
						$ct_gen->log(
													'market_error',
													'No market volume data available for ' . strtoupper($asset_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $ct_gen->key_to_name($key)
													);
