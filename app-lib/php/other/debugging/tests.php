...
if ( $runtime_mode == 'ui' ) {
	...
	if ( $ocpt_conf['dev']['debug'] == 'all' || $ocpt_conf['dev']['debug'] == 'alerts_charts' ) {
		...
		$charts_test_data = $ocpt_api->market($check_asset, $check_asset_params[0], $check_pairing_name, $check_asset_params[1]);
			...
			$ocpt_gen->app_logging('market_error', 'No chart / alert price data available', 'chart_key: ' . $key . '; market: ' . $check_asset . ' / ' . strtoupper($check_asset_params[1]) . ' @ ' . ucfirst($check_asset_params[0]) );
			}
			
			if ( $charts_test_data['24hr_prim_curr_vol'] == NULL || $charts_test_data['24hr_prim_curr_vol'] < 1 ) {
			$ocpt_gen->app_logging('market_error', 'No chart / alert volume data available', 'chart_key: ' . $key . '; market: ' . $check_asset . ' / ' . strtoupper($check_asset_params[1]) . ' @ ' . ucfirst($check_asset_params[0]) );
			}
	...
	if ( $ocpt_conf['dev']['debug'] == 'all' || $ocpt_conf['dev']['debug'] == 'texts' ) {
		...
		foreach ( $ocpt_conf['mob_net_txt_gateways'] as $key => $value ) {
			...
			$test_result = $ocpt_gen->valid_email( 'test@' . trim($value) );
		
				if ( $test_result != 'valid' ) {
				$ocpt_gen->app_logging( 'other_error', 'email-to-mobile-text gateway '.trim($value).' does not appear valid', 'key: ' . $key . '; gateway: ' . trim($value) . '; result: ' . $test_result );
				}
			...
			foreach ( $coin_val['pairing'] as $pairing_key => $pairing_val ) {
				...
				foreach ( $pairing_val as $key => $value ) {
					...
					$markets_test_data = $ocpt_api->market( strtoupper($coin_key) , $key, $value, $pairing_key);
						...
						$ocpt_gen->app_logging('market_error', 'No market price data available for ' . strtoupper($coin_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $ocpt_gen->snake_case_to_name($key) );
						}
					
						if ( $markets_test_data['24hr_prim_curr_vol'] == NULL || $markets_test_data['24hr_prim_curr_vol'] < 1 ) {
						$ocpt_gen->app_logging('market_error', 'No market volume data available for ' . strtoupper($coin_key) . ' / ' . strtoupper($pairing_key) . ' @ ' . $ocpt_gen->snake_case_to_name($key) );
						}
