...
if ( $runtime_mode != 'cron' && !stristr( php_sapi_name() , 'fcgi') && $pt_conf['dev']['ignore_php_fpm_warning'] != 'yes' ) {
...
if ( $pt_cache->update_cache($base_dir . '/cache/events/scan-htaccess-security.dat', 60) == true ) {
	...
	$htaccess_cache_test = trim( @$pt_cache->ext_data('url', $htaccess_cache_test_url, 0) ); 
	
	// plugins check
	$htaccess_plugins_test_url = $base_url . 'plugins/htaccess_security_check.dat';

	$htaccess_plugins_test = trim( @$pt_cache->ext_data('url', $htaccess_plugins_test_url, 0) ); 
...
$pt_cache->save_file($base_dir . '/cache/events/scan-htaccess-security.dat', $pt_gen->time_date_format(false, 'pretty_date_time') );
