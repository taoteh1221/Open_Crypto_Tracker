...
if ( $oct_cache->update_cache($base_dir . '/cache/events/scan-htaccess-security.dat', 60) == true ) {
	...
	$htaccess_cache_test = trim( @$oct_cache->ext_data('url', $htaccess_cache_test_url, 0) ); 
	
	// plugins check
	$htaccess_plugins_test_url = $base_url . 'plugins/htaccess_security_check.dat';

	$htaccess_plugins_test = trim( @$oct_cache->ext_data('url', $htaccess_plugins_test_url, 0) ); 
...
$oct_cache->save_file($base_dir . '/cache/events/scan-htaccess-security.dat', $oct_gen->time_date_format(false, 'pretty_date_time') );
