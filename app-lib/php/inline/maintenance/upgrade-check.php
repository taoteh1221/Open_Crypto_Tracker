 ...
 */
	...
	if ( isset($ct_conf['comms']['upgrade_alert']) && $ct_conf['comms']['upgrade_alert'] != 'off' && $ct_cache->update_cache($base_dir . '/cache/vars/upgrade_check_latest_version.dat', (1440 + $daily_tasks_offset) ) == true ) {
	...
	$app_bug_fixes = $app_version_array[2];
		...
		// EVENTUALLY PUT UI ALERT LOGIC HERE
			...
			// With offset, to try keeping daily recurrences at same exact runtime (instead of moving up the runtime daily)
			if ( $ct_cache->update_cache($base_dir . '/cache/events/upgrade_check_reminder.dat', ( $ct_conf['comms']['upgrade_alert_reminder'] * 1440 ) + $daily_tasks_offset ) == true ) {
