...
if ( stristr($ct['conf']['currency']['bitcoin_primary_currency_exchange'], 'bitmex_') != false ) {
...
$ct['conf']['currency']['bitcoin_primary_currency_exchange'] = 'futures_mrkts_not_allowed'; // DISABLE
...
if ( isset($ct['opt_conf']['bitcoin_preferred_currency_markets'][ $ct['conf']['currency']['bitcoin_primary_currency_pair'] ]) ) {
$ct['opt_conf']['bitcoin_preferred_currency_markets'][ $ct['conf']['currency']['bitcoin_primary_currency_pair'] ] = $ct['conf']['currency']['bitcoin_primary_currency_exchange'];
}
...
// so move the default $ct['conf']['currency']['bitcoin_primary_currency_pair'] / $ct['conf']['currency']['bitcoin_primary_currency_exchange'] values into their own chart/alerts related variables,
// before dynamic updating of $ct['conf']['currency']['bitcoin_primary_currency_pair'] / $ct['conf']['currency']['bitcoin_primary_currency_exchange']
$ct['default_bitcoin_primary_currency_pair'] = $ct['conf']['currency']['bitcoin_primary_currency_pair']; 
$ct['default_bitcoin_primary_currency_exchange'] = $ct['conf']['currency']['bitcoin_primary_currency_exchange'];
...
// NON-ADMIN UI etc
    ...
    $ct['conf']['currency']['bitcoin_primary_currency_pair'] = $ct['sel_opt']['prim_currency_mrkt_standalone'][0]; 
        ...
        $ct['conf']['currency']['bitcoin_primary_currency_exchange'] = $ct['asset']->btc_mrkt($ct['sel_opt']['prim_currency_mrkt_standalone'][1] - 1);
        }
        else {
        $ct['conf']['currency']['bitcoin_primary_currency_exchange'] = $ct['asset']->btc_mrkt(0);
        }
    ...
    // MUST RUN !BEFORE! $ct['asset']->btc_mrkt() CALL BELOW, OR INCORRECT VALUE DETERMINED FOR $ct['asset']->btc_mrkt() CALL
    $ct['conf']['currency']['bitcoin_primary_currency_pair'] = $ct['sel_opt']['prim_currency_mrkt'][0]; 
    
    
    	// (we go by array index number here, rather than 1 or higher for html form values)
        if ( $ct['sel_opt']['prim_currency_mrkt'][1] > 0 ) {
        $ct['conf']['currency']['bitcoin_primary_currency_exchange'] = $ct['asset']->btc_mrkt($ct['sel_opt']['prim_currency_mrkt'][1] - 1);
        }
        else {
        $ct['conf']['currency']['bitcoin_primary_currency_exchange'] = $ct['asset']->btc_mrkt(0);
        }
...
$ct['sel_opt']['sel_btc_pair_id'] = $ct['conf']['assets']['BTC']['pair'][ $ct['conf']['currency']['bitcoin_primary_currency_pair'] ][ $ct['conf']['currency']['bitcoin_primary_currency_exchange'] ];

$ct['sel_opt']['sel_btc_prim_currency_val'] = $ct['api']->market('BTC', $ct['conf']['currency']['bitcoin_primary_currency_exchange'], $ct['sel_opt']['sel_btc_pair_id'])['last_trade'];

    
    // Log any Bitcoin market errors
    if ( !$ct['conf']['assets']['BTC']['pair'][ $ct['conf']['currency']['bitcoin_primary_currency_pair'] ] ) {
    	
    $ct['gen']->log(
    			'conf_error',
    			'primary-bitcoin-markets-config.php bitcoin_primary_currency_pair variable not properly set', 
    			'bitcoin_primary_currency_pair: ' . $ct['conf']['currency']['bitcoin_primary_currency_pair'] . ';'
    			);
    
    }
    elseif ( !$ct['conf']['assets']['BTC']['pair'][ $ct['conf']['currency']['bitcoin_primary_currency_pair'] ][ $ct['conf']['currency']['bitcoin_primary_currency_exchange'] ] ) {
...
$ct['conf']['assets']['MISCASSETS']['name'] = 'Misc. Assets (' . strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair']) . ')';
$ct['conf']['assets']['ALTNFTS']['name'] = 'Alternate NFTs (' . strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair']) . ')';
}
...
$ct['gen']->log(
    			'market_error',
    			'primary currency conversion market minimum value not met (for BTC / '.strtoupper($ct['conf']['currency']['bitcoin_primary_currency_pair']).' @ '.$ct['conf']['currency']['bitcoin_primary_currency_exchange'].', value = ' . ( isset($ct['sel_opt']['sel_btc_prim_currency_val']) ? $ct['sel_opt']['sel_btc_prim_currency_val'] : 'not set' ) . ')'
			);
